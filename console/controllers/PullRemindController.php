...
class PullRemindController extends Controller
{
    ...
    {
        // 获取订阅的管理员列表
        $list = Yii::$app->notifyService->notifySubscriptionConfig->findAllWithMember();
        /** @var NotifySubscriptionConfig $item */
        foreach ($list as $item) {
            Yii::$app->notifyService->notify->pullRemind($item);
        }
    }
    ...
    public function actionDingTalk()
    {
        ...
        foreach ($list as $item) {
            if (!empty($item->manager)) {
                $result = Yii::$app->notifyService->notify->pullRemind($item, SubscriptionAlertTypeEnum::DINGTALK);
    ...
    public function wechat()
    {
        ...
        foreach ($list as $item) {
            if (!empty($item->manager)) {
                $result = Yii::$app->notifyService->notify->pullRemind($item, SubscriptionAlertTypeEnum::WECHAT);
