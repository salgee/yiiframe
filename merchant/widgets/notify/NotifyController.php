...
{
    protected $view = '@merchant/widgets/notify/views/';
    ...
    public function actionAnnounceView($id)
    {
        ...
        // 设置公告为已读
        Yii::$app->notifyService->notify->read(Yii::$app->user->id, [$model->notify_id]);
    ...
    public function actionMessage()
    {
        ...
        if ($data = $dataProvider->getModels()) {
            ...
            // 设置消息为已读
            !empty($ids) && Yii::$app->notifyService->notify->read(Yii::$app->user->id, $ids);
        }
    ...
    {
        Yii::$app->notifyService->notify->readAll(Yii::$app->user->id);
