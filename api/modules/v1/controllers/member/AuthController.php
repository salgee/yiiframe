...
class AuthController extends UserAuthController
{
    ...
    public function actionCreate()
    {
        ...
        /** @var Auth $model */
        if (!($model = Yii::$app->memberService->auth->findByMemberIdOauthClient($oauthClient, $member_id))) {
        ...
        // 更改用户信息
        if ($member = Yii::$app->memberService->member->get($model->member_id)) {
    ...
    {
        $member_id = Yii::$app->user->identity->member_id;
        $member = Yii::$app->memberService->member->get($member_id);
        if (empty($member['mobile']) && Yii::$app->memberService->auth->getCountByMemberId($member_id) == 1) {
    ...
    public function actionIsBinding()
    {
        ...
        $model = Yii::$app->memberService->auth->findOauthClientByMemberId($oauthClient, Yii::$app->user->identity->member_id);
        if ($model) {
            return [
